        ...
        Console.WriteLine("Inserting data...");
                ...
                using (var file = new System.IO.StreamReader("students.csv"))
                {
                    while(!file.EndOfStream)
                    {
                        ...
                        studsql = string.Format(@"Insert Into 
                                                        Students(NetID,StudFName,StudLName)
                                                        Values('{0}', '{1}', '{2}');", netid, firstname.Replace("'", "''"), lastname.Replace("'", "''"));



                        data.ExecuteActionQuery(studsql);
                        Console.WriteLine(studsql);
                    }
                }
                ...
                using (var file = new System.IO.StreamReader("courses.csv"))
                {
                    while (!file.EndOfStream)
                    {
                        ...
                        string typesql = string.Format(@"Insert Into
                                                         Courses(CRN,Department,CourseNum,Semester,Year,Type,Day,Time,ClassSize)
                                                         Values({0}, '{1}', {2}, '{3}', {4}, '{5}', '{6}', '{7}', {8});", crn, department, coursenum, semester, year, type, day, time, classsize);
                        data.ExecuteActionQuery(typesql);
